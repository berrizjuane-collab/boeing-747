...
**Ronda 3 en ejecución sobre `main@1034fd1`, mediante PR borrador #2 (`agent/plan3-phase0`). Fase 0 cerrada; Fase A implementada en el candidato sistémico `b32030a` y en validación visual.** La instrumentación móvil, la consolidación runtime del tren, el blindaje de contraste de S3 y los pesos de carga ya tienen evidencia visual y numérica en el run [Final visual QA #16](https://github.com/berrizjuane-collab/boeing-747/actions/runs/31782584163). El [run #18](https://github.com/berrizjuane-collab/boeing-747/actions/runs/31788579338) validó el recorrido completo y produjo las 23 capturas narrativas de A, pero el job de screenshots terminó en la acción del arnés que debía cambiar el tier antes de ejecutar las sondas deterministas. A1–A4 y A6 permanecen `[~]` hasta que el siguiente run aporte el reporte y todas las comparativas exigidas; no se habilita Fase B antes de ese cierre.
...
|---|---|---|---|---|
| 1 | Alta | Tone mapping es **AgX, no ACES** — `<ToneMapping/>` sin prop `mode` | [~] | `ToneMappingMode.ACES_FILMIC` explícito y lectura runtime del efecto añadida. Run #18 produjo comparativas S1/S6; falta que el reporte final adjunte el modo efectivo junto a ellas. |
| 2 | Alta | `scene.environment` queda **nulo desde S7** | [~] | Propiedad centralizada y puente de barrido 0–100 % añadido. Las capturas 0,94/0,96 del run #18 no muestran pop de iluminación; falta evidencia runtime 101/101 en el reporte final. |
| 3 | Alta | Calibración de HDRI **dominada por el disco solar**; medianas descalibradas 3,31× | [ ] | |
| 4 | Media | Grading pre-tone-mapping con umbrales LDR sobre buffer HDR | [~] | `SectionGrade` movido después de ACES. La métrica histórica del frame completo quedó contaminada por UI y por el cambio simultáneo de tone mapping; el candidato `b32030a` agrega un probe grading off/on en el mismo build, frame y ROI de S6. |
| 5 | Media | `SectionGradeEffect.strength` congelado en `0.5` | [~] | Fuerza authored por sección e interpolada; test unitario verde y probe determinista off/on listo. Falta evidencia visual y cromática final de S6. |
| 6 | Media | **Sin antialiasing de ningún tipo** | [~] | SMAA explícito en los tres tiers y sonda A/B `?aa=off`. Run #18 mejoró la distribución del gradiente de silueta; falta costo medido y par determinista del reporte final. |
...
| # | Ítem | Verificación requerida | Estado | Evidencia |
|---|---|---|---|---|
| A1 | Aplicar la curva de tone mapping que el proyecto declara (bug #1) | Lectura en runtime del modo efectivo, más comparativa antes/después en el **mismo** encuadre de S1 y S6, con medición de saturación y contraste | [~] | Candidato fuerza `ACES_FILMIC=6`. Run #16→#18, S1: saturación **0,295287→0,227496**, contraste **6,78788→6,17949**; S6: saturación **0,317512→0,283886** a 0,86 y **0,327608→0,314226** a 0,88, contraste **39,6→43,2** y **40,2→44,4**. Falta asociar `mode=6` a las capturas en el reporte final. |
| A2 | Grading a espacio display o umbrales recalibrados para HDR; resolver `strength` (bugs #4, #5) | El key de grading de S6 (`#E89B6C`) es **medible en los highlights** de S6, no sólo en sus sombras | [~] | Orden display-space y strength S6 `0,64` implementados. El before/after histórico no aísla grading porque también cambia ACES; `b32030a` mide grading off/on sobre exactamente el mismo frame, tier Low y ROI sin paneles, y exige desplazamiento de highlights hacia `#E89B6C`. Resultado CI pendiente. |
| A3 | Antialiasing (bug #6) | Comparativa al mismo encuadre y resolución de la silueta contra el cielo, con medición de gradiente de borde. Costo en ms declarado contra `PLAN.md §7.2` | [~] | SMAA High/Medium/Low implementado. Run #16→#18 en Mobile Low: p50 del gradiente **16→13**, p95 **79→74** y píxeles intermedios **95,322 %→97,133 %**, consistente con una silueta más suave. Falta el costo A/B y el par aislado del reporte final. |
| A4 | `scene.environment` nunca nulo (bug #2) | Barrido de los 101 puntos: no nulo en todos. Capturas a 0,94 y 0,96 sin pop de iluminación | [~] | Un solo owner enlaza golden/high/cabin/sunset. Run #18 produjo 0,94 y 0,96 con el mismo IBL sunset y sin pop visible; falta el barrido automatizado 101/101 del reporte final. |
...
| A6 | **Recaptura de línea base completa** | Las 27 capturas de `qa:visual` regeneradas y archivadas como la nueva referencia | [~] | Run #18 archivó las **23 PNG narrativas**, incluidas 0,94/0,96, en el artefacto `9216046019` (`sha256:6a61d48505efae45fd1f2fe7c38fc3fa98a6b3a8e6b07ad524341b2edc8dc8f0`). La matriz final es de **27 PNG**: 23 narrativas + 2 de equivalencia del tren + 2 de grading A/B; las cuatro deterministas quedaron después del punto donde falló el arnés y se recapturan en el siguiente run. |
