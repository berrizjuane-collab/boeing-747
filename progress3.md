...
**Ronda 3 en ejecución sobre `main@1034fd1`, mediante PR borrador #2 (`agent/plan3-phase0`). Fase 0 cerrada; Fase A implementada en el candidato sistémico `8fd5d5f` y en validación visual.** La instrumentación móvil, la consolidación runtime del tren, el blindaje de contraste de S3 y los pesos de carga ya tienen evidencia visual y numérica en el run [Final visual QA #16](https://github.com/berrizjuane-collab/boeing-747/actions/runs/31782584163). A1–A4 y A6 permanecen `[~]` hasta que el nuevo run aporte las comparativas y mediciones exigidas; no se habilita Fase B antes de ese cierre.
...
|---|---|---|---|---|
| 1 | Alta | Tone mapping es **AgX, no ACES** — `<ToneMapping/>` sin prop `mode` | [~] | `ToneMappingMode.ACES_FILMIC` explícito y lectura runtime del efecto añadida; falta comparativa visual S1/S6 del run candidato. |
| 2 | Alta | `scene.environment` queda **nulo desde S7** | [~] | Propiedad centralizada y puente de barrido 0–100 % añadido; falta evidencia runtime 101/101 y capturas 0,94/0,96. |
| 3 | Alta | Calibración de HDRI **dominada por el disco solar**; medianas descalibradas 3,31× | [ ] | |
| 4 | Media | Grading pre-tone-mapping con umbrales LDR sobre buffer HDR | [~] | `SectionGrade` movido después de ACES; falta medir la distancia cromática de highlights antes/después. |
| 5 | Media | `SectionGradeEffect.strength` congelado en `0.5` | [~] | Fuerza authored por sección e interpolada; test unitario verde. Falta evidencia visual de S6. |
| 6 | Media | **Sin antialiasing de ningún tipo** | [~] | SMAA explícito en los tres tiers y sonda A/B `?aa=off`; falta gradiente antes/después y costo medido por CI. |
...
| # | Ítem | Verificación requerida | Estado | Evidencia |
|---|---|---|---|---|
| A1 | Aplicar la curva de tone mapping que el proyecto declara (bug #1) | Lectura en runtime del modo efectivo, más comparativa antes/después en el **mismo** encuadre de S1 y S6, con medición de saturación y contraste | [~] | Candidato fuerza `ACES_FILMIC=6` y publica modo efectivo por captura. Comparativa contra run #16 pendiente. |
| A2 | Grading a espacio display o umbrales recalibrados para HDR; resolver `strength` (bugs #4, #5) | El key de grading de S6 (`#E89B6C`) es **medible en los highlights** de S6, no sólo en sus sombras | [~] | Orden display-space y strength S6 `0,64` implementados; reporte calcula RGB medio del p90 y distancia a `#E89B6C`. Resultado CI pendiente. |
| A3 | Antialiasing (bug #6) | Comparativa al mismo encuadre y resolución de la silueta contra el cielo, con medición de gradiente de borde. Costo en ms declarado contra `PLAN.md §7.2` | [~] | SMAA High/Medium/Low implementado. Métrica aislada sobre la silueta Mobile Low sin grano y sonda A/B de costo listas; resultado CI pendiente. |
| A4 | `scene.environment` nunca nulo (bug #2) | Barrido de los 101 puntos: no nulo en todos. Capturas a 0,94 y 0,96 sin pop de iluminación | [~] | Un solo owner enlaza golden/high/cabin/sunset; barrido automatizado y capturas 0,94/0,96 añadidos. Resultado CI pendiente. |
...
| A6 | **Recaptura de línea base completa** | Las 25 capturas de `qa:visual` regeneradas y archivadas como la nueva referencia | [~] | Matriz ampliada de 23 a **25 PNG** con probes 0,94/0,96; artefacto y reporte aún pendientes del run candidato. |
