...
**Auditoría de Fase 9:** se revisó todo `src/` buscando nombres de aerolíneas reales, libreas o marcas (Emirates, Qatar, Singapore Airlines, Lufthansa, Boeing, "747", etc.) fuera de comentarios de código que discuten la decisión de producto en sí. No se encontró ninguna referencia — la única marca que aparece en la copy renderizada es la ficticia `MERIDIAN` (`content.ts`'s `BRAND_NAME`). Confirma que la decisión de §12.4 está implementada de punta a punta, no sólo declarada.
...
| **3** | ✅ Completa | Pipeline `gltf-transform` construido y probado (prune→dedup→weld→**instance**→Draco→KTX2 — `instance` se agregó sobre lo planeado en §6.3, ver PROGRESS.md). Entorno de Secciones 1–3 construido. `exterior.glb` real procesado e integrado en la app (2026-08-05). KTX2/Basis genuino, HDRI reales de S1/S3 (generadas proceduralmente con Cycles/Nishita) y verificación visual completa en navegador real, cerrados 2026-08-06 | 0, 2 |
| **4** | ✅ Completa | **Spike del umbral (S4) resuelto y re-verificado contra geometría real** (exterior + interior, 2026-08-05): disolución radial, cross-fade de luz, rampa de exposición, registración espacial. Geometría de detalle del umbral (marco de puerta en ambos portales) cerrada 2026-08-06. Bloom real sigue diferido a Fase 7 a propósito (necesita el pipeline de post-procesado, que es alcance de esa fase, no de ésta) | 3 |
| **5** | ✅ Completa | Interior (S5): las **3 zonas de v1** recorridas con geometría real de ambos lados. Instancing de asientos vía `EXT_mesh_gpu_instancing` en el pipeline. Iluminación de cabina (fill PMREM + 2 spots con sombra), LOD de corredor (*fade-out* real de opacidad) y mesetas de easing por zona implementados y verificados visualmente en navegador real, cerrada 2026-08-06 | 4 |
| **6** | ✅ Completa | S6/S7 (umbral inverso, retroceso a vista general, atardecer, outro, footer), HDRI real de atardecer, sistema de overlays con tipografía y grilla de tercios, hotspots con dwell-gating, pantalla de carga con progreso ponderado, nav fijo — implementados y verificados visualmente, cerrada 2026-08-06. Coordenadas 3D de 4 hotspots de economy quedan como afinado visual pendiente, documentado en PROGRESS.md | 5 |
| **7** | ✅ Completa | Post-proceso vía `postprocessing` (no el `EffectComposer` nativo): ACES en todos los tiers, bloom, DoF (S5/desktop-high), viñeta+grano fusionados, godrays en el umbral (desktop-high), grading por sección según §10.1. Dos regresiones reales encontradas y corregidas al montar el composer (stats del HUD, exposición) — cerrada 2026-08-07 | 6 |
| **8** | ✅ Completa | Tiering de 3 niveles con detección automática + override manual, guardarraíl de carga del interior antes de S4, `prefers-reduced-motion` completo, detección de WebGL2 + fallback estático, accesibilidad (headings, `aria-hidden`, contraste AA en S3). Pruebas en iOS/Safari/Android real quedan sin verificar — no realizable en este entorno headless, documentado como limitación honesta, no como hecho — cerrada 2026-08-07 | 7 |
| **9** | ✅ Completa | **Verificación de datos (§9)** cerrada: cada cifra cruzada contra fuentes primarias Airbus/Rolls-Royce/Engine Alliance por convergencia de múltiples fuentes independientes (el acceso directo `WebFetch` a airbus.com/rolls-royce.com sigue bloqueado en este entorno, ahora por egress de red en vez de HTTP 403 — mismo resultado práctico, confirmado de nuevo, no resuelto). Conflicto de empuje de 1.208 kN resuelto (total de 4 motores, no cifra por motor); motor Trent 900 reconfirmado y mantenido sin cambios; cifra específica de aerolínea (615 pax) confirmada excluida de la copy; rango de alcance corregido a 14.800–15.200 km; distinción ancho de fuselaje/ancho de cabina documentada. La copy final ya existía en `content.ts` desde Fase 6 (estructura completa por sección) — Fase 9 verificó los valores y volteó `verified: false → true`, no reescribió la redacción desde cero. Atribución CC BY 4.0 confirmada renderizada en el footer, tanto en la versión 3D como en el fallback estático. Auditoría de IP sin hallazgos (ninguna marca real fuera de la ficticia MERIDIAN). Cerrada 2026-08-07 | — |
