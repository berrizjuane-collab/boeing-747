...
**Ronda 3 en ejecución sobre `main@1034fd1`, mediante PR borrador #2 (`agent/plan3-phase0`). Fase 0 cerrada; A1–A6 ya tienen la evidencia exigida en el [run #19](https://github.com/berrizjuane-collab/boeing-747/actions/runs/31793196464), pero el cierre global de Fase A queda condicionado a repetir el oráculo F0 en el candidato QA `a2081f4`.** El producto pasó 23/23 encuadres, 101/101 muestras de entorno, grading A/B, instrumentación SMAA y el video completo sin errores. La única aserción roja fue una diferencia de precisión de **1 píxel / 1 LSB** en el tren al comparar matrices GPU de la jerarquía contra vértices Float32 horneados del merge. No se aceptó como «suficientemente igual»: el probe ahora compara draw calls separados vs. concatenados desde exactamente los mismos vértices horneados, mientras el test unitario conserva la prueba independiente de transforms authored. Fase B sigue bloqueada hasta obtener **0/0** en esa repetición acotada.
...
| 02 | **Fusión de las 115 mallas de `LandingGear` en runtime** | Mobile Low hero **99 → ≤10** y Desktop High hero **148 → ≤36**, medidos. Y `01-hero.png` / `08-mobile-hero.png` **idénticos píxel a píxel** a los artefactos de la ronda 2 — cualquier diferencia es un bug | [x] | High **148→34** calls; Low **99→8**. Par determinista `qa-gear-source/merged.png`, mismo build/viewport/pipeline sin grano temporal: **0 píxeles distintos, delta máximo 0**. El diff bruto entre runs históricos no era un oráculo válido: High usa `Noise` dependiente de `time` (RMSE 0,00310 aun sin cambio de geometría); Low difirió en 3 de 329.160 píxeles entre runners, pero el toggle source/merged dentro del mismo runner fue exacto. |
