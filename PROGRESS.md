...
**Estado global: Fases 0–4 completas.** El bloqueo del exterior (más abajo, RESUELTO) se destrabó el 2026-08-05: el usuario adjuntó `A380.blend` de nuevo y autorizó subirlo al repo. **Fase 3 se cerró el 2026-08-06**: las tres condiciones que quedaban explícitas (verificación visual con navegador real, HDRI reales para S1/S3, KTX2/Basis genuino en el artefacto exterior) eran limitaciones del contenedor de esa sesión, no del código — la sesión siguiente corrió en un entorno con Blender, `ktx` instalable y navegador Playwright preinstalado, y las resolvió todas con evidencia verificada (ver la sección de Fase 3 más abajo). **Fase 4 se cerró el mismo día**: la geometría de detalle del umbral (marco de puerta en ambos portales) que quedaba como alcance no cubierto ya está implementada y verificada. Hay un adelanto real de Fase 5 (interior recorrido con geometría verdadera).
