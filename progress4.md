...
**Implementación en curso (sesión de implementación 1).** Fase 0₄: 3 de 4 ítems cerrados con evidencia (04-01/02/03); 04-04 declarado, cierra al final de Fase G. Fase G: en curso. Fases H, I, J: no comenzadas.

Las **carencias #14–#19** de `plan4.md §2` están todas abiertas (se cierran junto con la fase que las resuelve, no antes).
...
| 04-04 | Presupuesto de textura declarado **antes** de gastarlo (bug #18): tamaño por tier, VRAM, peso S0 previsto | Cifras escritas acá antes de que exista la textura, y comparadas con lo medido al cierre de G2, contra 350 / 200 / 120 MB de `PLAN.md §7.1` | [~] | **Declarado, pendiente de confirmar contra G2 real.** Tres `DataTexture` (albedo, normal, roughness) por el patrón de `aircraftSurfaceMaps.ts`: **high/mid 1024²** → 4.194.304 B crudos por mapa × 3 = 12.582.912 B, ×4/3 con mipmaps ≈ **16,78 MB** (4,8 % de 350 MB high; 8,4 % de 200 MB mid). **low 512²** → 1.048.576 B × 3 = 3.145.728 B crudos, ×4/3 ≈ **4,19 MB** (3,5 % de 120 MB). Tile de detalle cercano 64² (normal+roughness, mismo patrón de 2 mapas que `aircraftSurfaceMaps.ts`, sin albedo): ≈0,044 MB, despreciable. **Peso de red S0: 0 B** — procedural en cliente, mismo patrón que `aircraftSurfaceMaps.ts`, no pasa por `loadingWeights.ts`. Costo real es tiempo de generación (hitch), no bytes: estimado 80–200 ms por `plan3.md §3.2` (mismo perfil de 64²→1024² escalado ×256), a medir de verdad en G2. **Nota de alcance, ya señalada en el bug #18**: esto es el costo *incremental* de la textura nueva, no una auditoría de VRAM de escena completa — ningún script del repo suma eso hoy. |
