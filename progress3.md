...
**Fase 0 y Fase A completas (implementación, sesión 3).** Fase 0: los cuatro ítems de instrumentación y presupuesto, implementados y verificados con evidencia real. Fase A (sistémica): los seis ítems (A1–A6) implementados y verificados, con recaptura de línea base completa archivada. Fases B–F: no comenzadas — ver "Qué queda abierto" en el registro de sesiones. Bugs reales cerrados con evidencia: 1, 2, 4, 5, 6, 7, 8, 9, 10, 11, 12 (11 de 13); bug #13 parcialmente (pendiente de revalidar tras Fase B4); bug #3 sigue abierto (Fase B3).
...
Mobile Low se mantiene exactamente en el objetivo de Fase 0 (≤10) — SMAA se excluyó deliberadamente de ese tier, ver Fase A · A3. Desktop High queda en 37, por encima del `≤36` que Fase 0 había fijado antes de que existiera antialiasing en el presupuesto, pero con margen amplísimo contra el techo real (`<250`).
...
| 3 | Alta | Calibración de HDRI **dominada por el disco solar**; medianas descalibradas 3,31× | [ ] | Fase B3, no comenzada |
...
| 13 | Baja | `loadingWeights.ts` con pesos de HDRI obsoletos (high-altitude +43 %) | [~] | Fase 0 · ítem 04 corrigió los pesos contra el disco; el propio bug pide revalidar tras Fase B4 (que cambia los tres archivos de HDRI) — no cerrado del todo hasta esa revalidación |
